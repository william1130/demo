...
<table border="0" cellpadding="4" cellspacing="1"
	...
	<thead>
		...
		<tr>
			...
			<th rowspan="2">對應主機</th>
			<th rowspan=2>交易類別</th>
			<th>Process Code</th>
			<th>批號</th>
			<th>回應碼</th>
		</tr>
			...
			<tr class="<edstw:rowStyle value='${status.index}'/>">
				<td class="alignCenter">
					<c:if test="${item.showCardIso == 'Y'}">
						<a href="#"
							onclick="showIFrameDialog('<c:url value="/trans/queryIFESTransLogISO.do?entity.seqId=${item.seqId}&entity.maskCardNo=${item.maskCardNo}&entity.merchantId=${item.merchantId}&entity.approveCode=${item.approveCode}"/>',{width:1080, height:'520',title: 'ISO內容'});return false;"
				...
				<td class="alignCenter" rowspan="2"><c:out value="${item.hostAccord}"/></td>
				<td class="alignCenter" rowspan="2">
					<c:out value="${item.tranType}"/>
					
				</td>
				<td class="alignCenter"><c:out value="${item.procCode}"/></td>
				<td class="alignCenter"><c:out value="${item.batchNum}"/></td>
				<td class="alignCenter"><c:out value="${item.respCode }" /><c:out value="${paramBean.responseCodeNameMap[item.respCode]}"/></td>
