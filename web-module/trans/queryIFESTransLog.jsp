...
<body>
	<div id="mainContentBlock" style="margin:0 auto;width:100%;">
		<div id="progHeader">
			<div id="progTitle">IFES Log查詢</div>
		</div>
		<jsp:include page="/jsp/messages.jsp" />
		<s:form action="queryIFESTransLog" style="text-align: center;">
			<%-- 設定開啟分頁功能 --%>
			...
			<edstw:folding foldingId="criteriaBlock" description="查詢條件" >
				<fieldset>
					...
					<table border="0" align="center" cellpadding="4" cellspacing="0">
						...
						<tr>
							<td class="optional">對應主機</td>
							<td colspan="3">
								<s:select name="entity.hostAccord" list="#{'ALL':'全部','ATS':'ATS','HG':'HG','DCC':'DCC','ASM':'ASM','ESC':'ESC','TMS':'TMS','MPAS':'MPAS','ESVC':'ESVC','TRUS':'TRUST'}" onChange='changeHost(this);'/>
						...
						<tr>
							...
							<td><s:select name="entity.tranType" list="#attr.paramBean.ifesTransTypeNameList"
						...
						<tr>
							...
							<td colspan="5">
							<s:radio name="entity.sortFields" list="#{'tran_date':'交易日期', 'tran_time':'交易時間', 'card_num':'卡號', 'tran_amount':'交易金額', 'approve_code':'授權碼', 'resp_code':'回應碼', 'merchant_id':'特店代號', 'term_id':'端末代號'}" />
							</td>
						</tr>
						<tr>
	...
	<c:import url="/ajax/queryTranTypeOptionsFromHost.jsp"/>
	...
	function checkForm(event) {
		...
		if (validateForm(form)) {
			form.action = "<c:url value="/trans/queryIFESTransLog.do"/>"
			return true;
		} else {
			event.stop();
			return false;
	...
	function initialForm() {
		...
		checkUrl("<c:url value="/trans/checkingIFESShowCardNo.do"/>",
			function() {
				if (arguments[0]) {
		...
		checkUrl("<c:url value="/trans/checkingIFESShowISO.do"/>",
			function() {
				if (arguments[0]) {
	...
	function reloadPage(){
		location.href = '<c:url value="/trans/toQueryIFESTransLog.do"/>';
	}
	...
	function changeHost(event) {
		var targetHost = form['entity.tranType'];
		if(form['entity.hostAccord'].value != null)
			queryTranTypeOptionsFromHost(targetHost, form['entity.hostAccord'].value);
