...
<%-- <c:set var="resultList" value="${emvTagRecordForm.resultList}" /> --%>
	...
	<c:forEach var="item" items="${resultList}" varStatus="status">
		<tr class="<edstw:rowStyle value='${status.index}'/>">
			<td rowspan=2>
				...
				<a
				href="<c:url value='/emv/toModifyEmvTagRecord.do?entity.emvTag=${item.emvTag}&entity.sameValueFlag=${item.sameValueFlag }'/>"
				onclick="showIFrameDialog( this, {title: '<c:out value='EMV TAG維護'/>'}); return false;"
...
<s:form action="approveEmvTagRecordTemp" id="form1">
	...
	<s:hidden name="entity.uiLogAction" value="核准"/>
...
<s:form action="rejectEmvTagRecordTemp" id="form2">
	...
	<s:hidden name="entity.sameValueFlag" value=""/>
	<s:hidden name="entity.uiLogOther" value=""/>
	<s:hidden name="entity.uiLogAction" value="拒絕"/>
	<s:hidden name="entity.uiLogFunctionName" value="EMV晶片"/>
</s:form>
</c:if>
<c:if test="${resultList.size() == 0 }">
