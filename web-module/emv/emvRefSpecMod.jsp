...
<html>
	<head>
		<title><c:out value='參考規格修改'/></title>
		<%@include file="/jsp/header.jsp"%>	
	</head>
	<body>
		<div id="mainContentBlock" style="width:70%;">
			<div id="progHeader">
				<div id="progTitle" title="程式代碼 : <c:out value='${currentFunctionObj.id}'/>"><c:out value='參考規格修改'/></div>
			</div>
			<jsp:include page="/jsp/messages.jsp"/>
			<div id="dataBlock">
				<s:form action="modifyEmvRefSpec">
				<%-- <c:set var="entity" value="${emvRefSpecForm.entity}"/> --%>
				<s:hidden name="entity.specID"/>
					<s:hidden name="entity.uiLogAction" value="MODIDY"/>
					<s:hidden name="entity.uiLogFunctionName" value="EMV_SPEC"/>
					<s:hidden name="entity.uiLogOther" value="修改：參考規格[%{entity.specName}]"/>
					<fieldset>
						<legend>修改</legend>
				...
				<script type="text/javascript">
					...
					function checkForm( event )
					{
						...
						$j("input[name='entity.uiLogOther']").val("參考規格["+form['entity.specName'].value +"]");
					...
					function initialForm()
					{
						...
						$revisedMonitor.addField($j("input[name='entity.specName']"),"參考規格");
