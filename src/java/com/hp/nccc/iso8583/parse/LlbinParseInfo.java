...
public class LlbinParseInfo extends FieldParseInfo {
	...
	public <T> ISOValue<?> parse(int field, byte[] buf, int pos, ISO8583VO vo, Object custom, boolean isTraceEnable) throws ISO8583ParseException,
		...
		if (pos < 0) {
			throw new ISO8583ParseException(String.format("Invalid bin LLBIN field %d position %d", field, pos), pos,
					field);
		} else if (pos + 1 > buf.length) {
			throw new ISO8583ParseException(String.format("Insufficient bin LLBIN header field %d", field), pos, field);
		}
		final int l = decodeLength(buf, pos, 2);
		if (l < 0) {
			throw new ISO8583ParseException(String.format("Invalid bin LLBIN length %d pos %d", length, pos), pos,
					field);
		}
		if (l + pos + 1 > buf.length) {
			throw new ISO8583ParseException(String.format(
					"Insufficient data for bin LLBIN field %d, pos %d: need %d, only %d available", field, pos, l,
	...
	public <T> ISOValue<?> parseString(int field, byte[] buf, int pos, ISO8583VO vo, Object custom, boolean isTraceEnable) throws ISO8583ParseException,
			UnsupportedEncodingException {
		if (pos < 0) {
			throw new ISO8583ParseException(String.format("Invalid LLBIN field %d position %d",
                    field, pos), pos, field);
		} else if (pos+2 > buf.length) {
			throw new ISO8583ParseException(String.format("Insufficient LLBIN header field %d",
                    field), pos, field);
		}
		int length = decodeStringLength(buf, pos, 2);
		if (length < 0) {
			throw new ISO8583ParseException(String.format("Invalid LLBIN field %d length %d pos %d",
