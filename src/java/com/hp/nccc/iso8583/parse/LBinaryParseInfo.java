 ...
 *
 */
public class LBinaryParseInfo extends FieldParseInfo {
	/**
	 * @param t
	 * @param len
	 */
	public LBinaryParseInfo() {
		super(ISOType.LBINARY, 0);
	}
	...
	public <T> ISOValue<?> parse(int field, byte[] buf, int pos, ISO8583VO vo, Object custom, boolean isTraceEnable)
		...
		if (pos < 0) {
            throw new ISO8583ParseException(String.format("Invalid LBINARY field %d position %d",
                      field, pos), pos, field);
        }
        ...
        if (pos+length > buf.length) {
            throw new ISO8583ParseException(String.format(
                      "Insufficient data for LBINARY field %d of length %d, pos %d",
                field, length, pos), pos, field);
        }
		byte[] _v = new byte[length];
		System.arraycopy(buf, pos, _v, 0, length);
		if(isNested(reference, (String[])objs[1])){
			return new ISOValue<ISO8583Fields>(type, parseNestFields(_v, reference, vo, isTraceEnable, (String[])objs[1]), length, getDescription(), true);
		}else{
...
//			missing = length - ((byte[]) value.getValue()).length;
			...
			byte[] _v = this.writeNestFields((ISO8583Fields)value.getValue(), reference, vo, (String[])objs[1]);
			outs.write(_v);
...
//		if (type == ISOType.LBINARY && missing > 0) {
...
//		}
	...
	public <T> ISOValue<?> parseString(int field, byte[] buf, int pos, ISO8583VO vo, Object custom, boolean isTraceEnable)
		...
		if(isNested(reference, (String[])objs[1])){
			return new ISOValue<ISO8583Fields>(type, parseStringNestFields(binval, reference, vo, isTraceEnable, (String[])objs[1]), length, getDescription(), true);
		}else{
