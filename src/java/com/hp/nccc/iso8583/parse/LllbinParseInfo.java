...
public class LllbinParseInfo extends FieldParseInfo {
	...
	public <T> ISOValue<?> parse(final int field, final byte[] buf, final int pos, ISO8583VO vo, Object custom, boolean isTraceEnable)
		...
		if (pos < 0) {
			throw new ISO8583ParseException(String.format("Invalid bin LLLBIN field %d pos %d", field, pos), pos, field);
		} else if (pos + 2 > buf.length) {
			throw new ISO8583ParseException(String.format("Insufficient LLLBIN header field %d", field), pos, field);
		}
		final int l = decodeLength(buf, pos, 3);
		if (l < 0) {
			throw new ISO8583ParseException(String.format("Invalid LLLBIN length %d field %d pos %d", l, field, pos),
					pos, field);
		}
		if (l + pos + 2 > buf.length) {
			throw new ISO8583ParseException(String.format(
					"Insufficient data for bin LLLBIN field %d, pos %d requires %d, only %d available", field, pos, l,
	...
	public <T> ISOValue<?> parseString(final int field, final byte[] buf, final int pos, ISO8583VO vo, Object custom, boolean isTraceEnable)
			throws ISO8583ParseException, UnsupportedEncodingException {
		if (pos < 0) {
			throw new ISO8583ParseException(String.format("Invalid LLLBIN field %d pos %d",
                    field, pos), pos, field);
		} else if (pos+3 > buf.length) {
			throw new ISO8583ParseException(String.format("Insufficient LLLBIN header field %d",
                    field), pos, field);
		}
		final int l = decodeStringLength(buf, pos, 3);
		if (l < 0) {
			throw new ISO8583ParseException(String.format("Invalid LLLBIN length %d field %d pos %d",
                    l, field, pos), pos, field);
		} else if (l+pos+3 > buf.length) {
			throw new ISO8583ParseException(String.format(
                    "Insufficient data for LLLBIN field %d, pos %d", field, pos), pos, field);
		}
