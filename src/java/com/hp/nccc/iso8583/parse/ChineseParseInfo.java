 ...
 * 
 */
public class ChineseParseInfo extends FieldParseInfo {

	/**
	 * @param t
	 * @param len
	 */
	public ChineseParseInfo(int len) {
		super(ISOType.CHINESE, len);
	}
	...
	public <T> ISOValue<?> parse(int field, byte[] buf, int pos, ISO8583VO vo, Object custom, boolean isTraceEnable) throws ISO8583ParseException,
		...
		if (pos < 0) {
			throw new ISO8583ParseException(String.format("Invalid CHINESE field %d position %d", field, pos), pos,
					field);
		}
		if (pos + length > buf.length) {
			throw new ISO8583ParseException(String.format(
					"Insufficient data for CHINESE field %d of length %d, pos %d", field, length, pos), pos, field);
		}
		...
		if (isNested(this.reference , (String[])custom)) {
			...
			return new ISOValue<String>(type, new String(_v, getCharacterEncoding()), length, getDescription(),
					false);
		}
	}
	...
	public <T> void write(ISOValue<T> value, OutputStream outs, ISO8583VO vo, Object custom) throws IOException {
		...
		if (value.getValue() instanceof byte[]) {
			...
			byte[] _v = ((String) value.getValue()).getBytes(getCharacterEncoding());
			outs.write(_v);
		...
		if (type == ISOType.CHINESE && missing > 0) {
	...
	public <T> ISOValue<?> parseString(int field, byte[] buf, int pos, ISO8583VO vo, Object custom, boolean isTraceEnable)
		...
		if (pos < 0) {
			throw new ISO8583ParseException(String.format("Invalid CHINESE field %d position %d",
                    field, pos), pos, field);
		}
		if (pos+(length*2) > buf.length) {
			throw new ISO8583ParseException(String.format(
                    "Insufficient data for CHINESE field %d of length %d, pos %d",
		...
		if(isNested(this.reference , (String[])custom)){
			...
			return new ISOValue<String>(type, new String(binval, getCharacterEncoding()), binval.length, getDescription(), false);
		}
