...
import proj.nccc.atsLog.batch.process.AuthMonrpt0502ProcImpl;
...
/**
 * - 執行auth_monrpt_0502 (會員銀行之每日人工授權交易分析月報表(MGAUTH502))
 *
 */
@Log4j2
public class AuthMonrpt0502Proc extends BatchBaseMain {
	...
	public void process(String jobId,String user,String[] args) throws Exception {
		...
		try {
			if(super.init(jobId)) {
				...
				AuthMonrpt0502ProcImpl impl = new AuthMonrpt0502ProcImpl(jobId);
	...
	{
		AuthMonrpt0502Proc proc = new AuthMonrpt0502Proc();
		
		String jobId="MGAUTH502";
		String user="SYSTEM";
